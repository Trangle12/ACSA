...
class TransReID(nn.Module):
    ...
    def forward_features(self, x, mask, mask_ratio):
        B = x.shape[0]
        ...
        if mask:
            ...
            x, mask_, ids_restore = self.random_masking(patches,mask_ratio)

            mask_tokens = self.mask_token.repeat(x.shape[0], ids_restore.shape[1]+1 - x.shape[1], 1)
            x_ = torch.cat([x[:, 1:, :], mask_tokens], dim=1)
            x_ = torch.gather(x_, dim=1, index=ids_restore.unsqueeze(-1).repeat(1, 1, x.shape[2]))
            x = torch.cat([x[:, :1, :], x_], dim=1) # append cls token
            # keep_index = (mask_ == 0)  # [B, N], bool type
    ...
    def forward(self, x, mask=False,mask_ratio=0.75):

        if mask:
            x_mask = self.forward_features(x, mask, mask_ratio)
            x_mask_global = F.normalize(x_mask[:, 0])
            return x_mask_global
        else:
            x = self.forward_features(x, mask, mask_ratio)
            x_global = x[:, 0]
            x_global = self.bottleneck(x_global)
            bn_x = F.normalize(x_global)
            return bn_x
